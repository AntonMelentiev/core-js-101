...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const dotProuduct = x1 * x2 + y1 * y2;
  const magnitudeProduct = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(dotProuduct / magnitudeProduct);
}
...
function isPrime(num) {
  //   Got from https://stackoverflow.com/questions/188425/is-there-a-simple-algorithm-that-can-determine-if-x-is-prime
  ...
  if (num <= 1) { return false; } // negativ, 0 and 1 are not prime
  if (num < 4) { return true; } // 2 and 3 are both prime
  if ((num % 2) === 0) { return false; } // exclude even numbers
  if (num < 9) { return true; } // we have already excluded 4, 6, and 8
  if ((num % 3) === 0) { return false; } // exclude remaining multiples of 3
...
function toNumber(v, d) {
  if (Number.isNaN(parseFloat(v))) { return d; }
  if (typeof parseFloat(v) === 'number') { return parseFloat(v); }
